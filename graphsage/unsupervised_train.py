...
#whether load trained model
flags.DEFINE_boolean('isTrain', True, "whether train a new model or load a trained model")
flags.DEFINE_string('testfile_prefix', '', "the prefix of test dataset (the prefix of init and dynamic)")
flags.DEFINE_string('checkpoint_dir', '', "the model dir")
...
flags.DEFINE_string('base_log_dir', './output_data', 'base directory for logging and saving embeddings')
...
def dynamic_test(init_file, dynamic_file, flag_file):
    [G, feats, id_map, walks, class_map, node_flag, flag_no] = init_G(init_file, flag_file, FLAGS.feature_size)
    print(id_map)
    print(FLAGS.isTrain)
    raw_input()
    ...
    with open(dynamic_file, 'r') as infile:
        while True:
            ...
            update_G(G, feats, id_map, class_map, edges_added, node_flag, flag_no, FLAGS.feature_size)
            print("init succeed! num of edges in G is "+G.number_of_edges())
            raw_input()
            # construct walks
...
def main(argv=None):
    if FLAGS.isTrain:
        ...
        dynamic_file = FLAGS.testfile_prefix + "_dyn"
        flag_file = FLAGS.testfile_prefix+"_flag"
        dynamic_test(init_file, dynamic_file, flag_file)
