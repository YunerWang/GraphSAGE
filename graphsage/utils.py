...
# initialize the G
def init_G(initFile, flagFile, fea_size):
    # read node flag
    node_flag = {}
    flag_set = []
    flag_no = 0
    ...
    update_G(G, feats, id_map, class_map, edges_init, node_flag, flag_no, fea_size, False)
    ...
    return G, feats, id_map, walks, class_map, node_flag, flag_no
...
def update_G(G, feats, id_map, class_map, edges_added, node_flag, flag_no, fea_size, isTest=True):
    if not isTest:
        _test=False
        _val=False
        _test_removed=False
    ...
    for e in edges_added:
        ...
        tn = e[1]
        if fn not in G:
            fea_vec = get_random_features(fea_size)
            label_vec = [0 for i in range(flag_no)]
...
def test_generate_input():
    nwFile="/home/wangyun/repos/DNE/data/dataset_ori/dolphins_nw.dat"
    flagFile="/home/wangyun/repos/DNE/data/dataset_ori/dolphins_flag.dat"
    ratio=0.8
    feature_size=10
    dataname="dolphins"
